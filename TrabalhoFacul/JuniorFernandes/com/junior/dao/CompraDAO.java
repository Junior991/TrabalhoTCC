...
public class CompraDAO implements IDAO<Compra> {
    ...
    public void inserir(Compra compra) throws Exception {
        ...
        String sql = "INSERT INTO COMPRA (CODIGOFORNECEDOR, DATACOMPRA, VALORTOTAL, SITUACAO) VALUES (?, ?, ?, ?)";
        ...
        for (ItemCompra iv : compra.getItens()) {
            sql = "INSERT INTO ITEMCOMPRA (CODIGOPRODUTO, CODIGOCOMPRA, QUANTIDADE, VALORUNITARIO) VALUES (?, ?, ?, ?)";
    ...
    public void alterar(Compra compra) throws Exception {
        Conexao c = new Conexao();
        String sql = "UPDATE COMPRA SET CODIGOFORNECEDOR=?, DATACOMPRA=?, VALORTOTAL=?, SITUACAO=? WHERE CODIGO=?";
        ...
        for (ItemCompra iv : compra.getItensRemover()) {
            sql = "DELETE FROM ITEMCOMPRA WHERE CODIGO=?";
            ps = c.getConexao().prepareStatement(sql);
        ...
        for (ItemCompra iv : compra.getItens()) {
            if (iv.getCodigo() == 0) {
                ...
                sql = "UPDATE ITEMCOMPRA SET CODIGOPRODUTO=?, CODIGOCOMPRA=?, QUANTIDADE=?, VALORUNITARIO=? WHERE CODIGO=?";
                ps = c.getConexao().prepareStatement(sql);
                ps.setInt(1, iv.getProduto().getCodigo());
                ps.setInt(2, iv.getCompra().getCodigo());
                ps.setInt(3, iv.getQuantidade());
    ...
    public ArrayList<Compra> listarTodos() throws Exception {
        ...
        String sql = "SELECT * FROM COMPRA ORDER BY DATACOMPRA DESC";
        PreparedStatement ps = c.getConexao().prepareStatement(sql);
        ...
        while (rs.next()) {
            ...
            String sqlItem = "SELECT * FROM ITEMCOMPRA WHERE CODIGOCOMPRA=?";
